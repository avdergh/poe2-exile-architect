...
def _restore(engine: Any, snapshot: str, expected: str) -> bool:
    try:
        engine.load_build_xml(snapshot)
        return build_state_hash(engine.get_xml()) == expected
...
def preserved_state(engine: Any):
    ...
    try:
        ...
        restored = _restore(engine, snapshot, state_hash)
        if not restored:
            setattr(engine, _RECOVERY, True)
...
def read_only_search(function):
    ...
    def guarded(engine, *args, **kwargs):
        ...
        with lock() if callable(lock) else nullcontext():
            ...
            try:
                restored = build_state_hash(engine.get_xml()) == state_hash
...
def evaluate_items(
    ...
    if build_state_hash(engine.get_xml()) != before:
