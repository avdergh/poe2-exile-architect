...
辅助实际应用已验证、结构与约束完整，且原生PoB明确识别数值模型缺口时，可按 capability gap
继续；已知 Mana/Spirit 超限、缺失读数和测量错误仍须处理。最终采纳档位遵循当前回执。
代理与Herald不能把负载攻击速度或零DPS当触发频率。持续/耐久/持续伤害物体缺少其时间和持续
伤害模型时，正的普通武器命中读数仍不能排序整个配套；保留明确的模型缺口，以unknown候选继续。
未建模不等于零收益或低技能价值。Agent可据机制和实测锚点给情景DPS范围，明确覆盖、重叠和
重复计数；报告分开显示PoB部分与粗估，粗估不替代合法性检查或Judge/reward数值。
