...
def build_researcher_prompt_package(
    ...
    *,
    ...
    system = (
        ...
        "- Mandatory semantic edges: every case must propose at least 2 resolver-backed "
        "semantic_edges whose endpoints both carry the STEP 3 resolution evidence. Derive them "
        "from the confirmed causal/role structure (mechanic chain generator->payoff, "
        "unique_enabler->skill, failure risk, transition gate, modelability caveat). If the "
        "case genuinely supports no edge, state the reason in the safe review instead of "
        "inventing relationships or padding with weak co-occurrence.\n"
        ...
        "Source builds are not correctness authorities. Omit confirmed incompatible source "
        "supports from durable prose, components, packages and edges; do not create incompatibility "
        "notes or open-question records just to describe them. Use sourceSkillGroupReviews "
        "supportDisposition=excluded_incompatible only when the service confirms every support "
        "in that exact container is incompatible. Mixed groups remain packaged with only applicable "
        "supports. Exclusions are audit-only; unresolved or unknown applicability is not exclusion.\n"
        ...
        "supports unless an explicit source_coverage_gap/not_applicable exception applies, or "
        "verified source exclusions leave fewer applicable supports; cover those remaining supports "
        "without inventing replacements. "
        "passiveAscendancy requires an ascendancy_shell plus "
        "typed_payload.ascendancyResponsibilities tied to a passive that physically belongs to that "
        "ascendancy. gearRoles requires a gear_synergy record with node-referenced "
        "typed_payload.gearResponsibilities, or content-based rare-gear evidence "
        "(gearResponsibilities explicitly [], slot + target mods + roll pursuit in content; "
        "missing/null is not a declaration); "
        ...
        "strong_ranking_hint. A single sample is only case_observation. Every pattern must carry "
        "an agent_semantic_scope_review context requirement that confirms case_only or a "
        "conditional_transfer_hypothesis; typed sample/family/source counts, never wording in any "
        "language, authorize confidence.\n"
