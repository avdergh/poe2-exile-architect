...
解析到天赋 stable key 后，用 `graph_tool_query(tool_name="explain_graph_evidence",
...
- `get_gem.effectDetails` 按精确 effect 分别提供正文、类型约束和 statSet；由数值/模板投影的描述有
...
7. claim 成功后的任何结束路径都返回 `sampleId + safe outcome`；accepted 时附 safe acceptance 摘要：
   ...
   validation-only 的候选 recordId 不代表持久身份；验收后以 `get_research_write_receipt` 的
   ...
   recordId用 `query_research_memory(detail_level="record", response_profile="full")` 读回并核绑定，
