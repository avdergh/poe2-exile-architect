...
从本次 `start_generation_run.agentOutputDraftTemplate` 填写 prompt/candidate；顶层字段已有模板，不自行重建另一份列表。原样保留本次 `runContext`、`packetId`、`promptId/requestRef`、`experimentContext`，不得复用其他 run 的旧 `HumanReviewPacket`。内部对象使用 schema 的 camelCase；工具参数仍按各工具签名传入。
...
`query_research_memory` 条目覆盖授权与对照的全部receipt引用，另必须有实际 `get_freshness_report`
条目；`construct_research_execution_contract` 与 `validate_generation_blueprint` 引用匹配最终
