...
默认模式必须渐进查询
`query_research_memory(response_profile="create_compact")`，并在确认精确 Family 后调用
...
（查询 `query_learning_memory`、提交 `submit_learning_create_result`）。
...
5. 使用 `apply_build_mutation_batch(batch_kind=..., ...)` 把 Agent 已经决定的机械操作按职能拆成
   ...
   **箭袋（Quiver）不在任何批次白名单**，引擎把它映射为 `Weapon 2` 槽（completeness 层叫
   ...
   `pinnacle_readiness`；剧情阶段不得被其终局门槛驱动。
    质量收尾必须包含一次暗金评估轮：先调用 `relevant_uniques`（按活动主技能 scaling 匹配
    unique 与 unique jewel 候选），再结合 family 记录深读得到的 `unique:` 组件，用
    `search_uniques`/`get_unique` 读全文核对机制；对确有机制收益的候选用 `equip_item` /
    `equip_jewel` 实测 delta，数值以引擎为准，并用 `get_prices` 把核心暗金的价格/获取难度写入
    ...
    `equip_jewel` 提交；普通 rare jewel 仍用 `optimize_jewel`。注意语料 unique jewel 表缺
...
7. 调用一次 `inspect_generation_checkpoint(strict_mode=<本次反馈模式>)`。它按语义 `build_state_hash` 合并
...
召回上限）｜`query_public_learning_memory`（普通 Create 用，lesson 必须重新验证）｜
...
`plan_gear`/`optimize_item`/`scaffold_gear`/局部 `optimize_passives`｜
...
`start_generation_run`/`validate_generation_output`/`complete_generation_review`（Phase 5 run，
不依赖仓库工作目录）｜`save_final_build_artifact`（先于 complete_review）｜
`list_final_build_artifacts`/`load_final_build_artifact`｜`export_final_build_package`（普通单阶段
交付入口；单格式补导/诊断用 `export_final_pob_artifact`/`export_final_build_artifact`/
...
- `toolReferences`：每项为对象 `{toolName, queryRef, summary}`；其中
  `query_research_memory` 条目的 `queryRef` 必须覆盖 `researchMemoryUse.dedupeQueryRefs` 全部
  引用，且必须包含一条 `get_freshness_report` 条目（服务端硬性要求）
...
`start_generation_run` / `validate_generation_output` / `complete_generation_review` 按工作流
