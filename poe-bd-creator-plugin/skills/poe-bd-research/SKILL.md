...
| `--resume --run-ref REF` | 用 `get_research_run_status(run_ref=REF)` 恢复调度。 |
...
Controller 使用 `start_research_run / get_research_run_status / cleanup_research_run` 管理运行态。
...
调用 `start_research_run` 时按参数表传入请求。live collector 外层超时至少 10 分钟；超时按 runtime
...
`cleanup_research_run(run_ref=<runRef>)`；还须确认 write receipt/legacy receipt 已保存并完成 ledger
...
- 批准知识补录：结束旧 Worker assignment，通过 `start_research_run` 传
