...
1. 用`get_research_followup_status(run_ref, sample_id)`分页读取缺口，保留`gapRef`、`revision`、
...
3. 读取`get_research_write_receipt`及其选中记录详情，核对新证据是否真正回答原问题与适用场景；
   回执默认summary只含分页ID/指纹；按`pagination.nextCursor`读全，`detail="records"`读取完整映射与
   当时保存的摘要；正文按实际recordId用`query_research_memory`的record模式读回并核来源与当前投影。
   `detail="diagnostics"`配合返回的`diagnosticSections.section`分页取审计明细，不能把未返回项当不存在。
   通过`submit_research_gap_review`提交稳定`request_id`和当前`expected_revision`，逐gap给出
...
用户授权重取时，调用`reacquire_research_source(run_ref, sample_id, request_id)`。工具仅重新发现
...
`get_research_followup_status(view="reacquisitions")`分页读取。已接受并完成收尾的子run会结束
...
保留的待确认事项时，可调用普通`cleanup_research_run`按锁定策略清理。活动lease、accepting和待
