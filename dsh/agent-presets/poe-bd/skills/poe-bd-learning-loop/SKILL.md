...
- 原始 code/XML 只传给 `mcp__poe_learning__intake_learning_case`。工具返回后不得把原文写入提示词、聊天、控制
...
1. 实际调用 `mcp__poe_learning__start_learning_campaign(operation_id=<唯一值>, case_limit=10)`。
2. 每次仅导入一个案例：
   - direct：`mcp__poe_learning__intake_learning_case(..., source_mode="direct", source=<原始输入>)`；
...
1. 调用 `mcp__poe_learning__load_learning_reference_case(campaign_id, case_id, claim_id, thread_id)`；响应不含 raw。
2. 串行使用 `mcp__poe_build__get_build`、`mcp__poe_build__list_skill_groups`、`mcp__poe_build__get_build_stats`、`mcp__poe_build__get_defenses`、
   `mcp__poe_build__inspect_build_completeness` 和必要的机制工具理解原 BD。不要把整角色镜像写入持久报告。
3. 用 `mcp__poe_knowledge__graph_tool_query` 的 search → resolve 两步确认升华、主技能和真正 Family-core 的次级技能。
   辅助、utility、普通防御/资源技能不能机械加入 Family。
4. 以现有 Research `identity_records` 形状调用 `mcp__poe_learning__submit_learning_profile`。`targetLevel` 必须来自活动
...
```text
使用 $poe-bd-create 执行本次 BlindCreatePacket。先调用 mcp__poe_learning__get_learning_create_packet 取得唯一输入；
...
1. 可再次用当前 compare claim 调用 `mcp__poe_learning__load_learning_reference_case`，读取必要的 reference 证据；记录
   临时安全笔记后，调用 `mcp__poe_build__load_final_build_artifact` 检查 generated artifact。
...
5. reference 更强时，把 gap 分类为固定七类之一。每个 Judge 附件必须
   ...
   `noRewardWrite=true`。
6. 调用 `mcp__poe_learning__submit_learning_comparison`。赢家来自 Comparator 的证据判断，不来自工具自动推导。
...
- DB-fit 知识使用现有 `$poe-bd-research`/Research typed proposals；不得把它改写成泛化 lesson 逃避
  Research schema。
- 跨维 Create 行为经验用当前 campaign/case/claim/thread 调用 `mcp__poe_learning__propose_learning_lesson`，必须
  `dbFit=false`，带版本、条件、排除、
  verification tasks 和安全 refs。成功后立即可供下一案例召回。
- 发现 Memory harmful/incorrect 时用同一 Learn claim 调用 `mcp__poe_learning__append_learning_memory_correction`。修正只能
...
- 小型确定性代码缺陷仅在 `codex/*` 分支最小修复并补测试；`main` 上暂停。产品/架构决定或证据
  不足写入 backlog。
- 最后调用 `mcp__poe_learning__complete_learning_case_feedback`。任何 Research、Memory 或 code mutation 都进入
  `rereview_pending`；`none` 直接完成；backlog 暂停。

条件复审仍在 Reference 任务，claim `rereview`。只复审本案例实际 mutation，不重新生成本案例，
不改变已提交 winner。调用 `mcp__poe_learning__submit_learning_rereview` 后才可开始下一案例。
...
保存最终趋势摘要后，调用
`mcp__poe_build__cleanup_completed_task_runtime(task_kind="learning_campaign", task_id=campaignId)`。它删除 campaign、
