...
lifecycle 和 `mcp__poe_build__evaluate_generation_candidate` 统一使用 `strict_mode=false`。默认仍运行完整计算，
...
`mcp__poe_build__optimize_supports`、`mcp__poe_build__optimize_item_sockets`、`mcp__poe_build__plan_item_sockets_batch` 使用
`background=true`，先保存返回的 `operationId`，再用 `mcp__poe_build__get_compute_operation` 取状态与完整结果。
...
不能声称已有跨重启恢复。取消使用 `mcp__poe_build__cancel_compute_operation`，`cancel_requested` 不是已停止；
...
1. 对终局/Boss 目标调用一次 `mcp__poe_build__apply_combat_profile`，选择符合用户目标的 tier，并传最新
   `expected_state_hash`；也可用 `mcp__poe_build__set_config` 设置相同 key。公共 `mcp__poe_build__set_config` 是只修改显式字段的PATCH；
   `mcp__poe_build__apply_combat_profile` 完整替换自己的Boss tier与六个布尔条件，false会清除旧值；二者返回最新stateHash。
...
2. 确定本次唯一的 offense group 和精确主动技能名：纯清图目标选 clear group，Boss/均衡目标选
   single-target group，用新鲜fingerprint的`mcp__poe_build__set_skill_group_state`锁定对应active effect。
...
3. 装备、天赋、珠宝、Rune 和 config 锁定后，重新生成最终 Support/Jewel/Socket 回执。
   ...
   - 等级 ≥90 的额外珠宝槽审计保留精确 `protected_node_ids`，以当前真实候选审查全部可达槽。
     正收益经 `mcp__poe_build__apply_next_jewel_socket_decision` 原子应用后，在新状态重新审计；受保护策略下的
   ...
   - Socket 使用`item_socket_review_v2`且必须对应实际装备。`socketed/partial_socketed`均是待应用
     方案，携原`craftReceiptRef`完成可信`mcp__poe_build__equip_item`后，后者才转为`partial_no_positive`，
...
Research 深读完成、最终摘要形成后，按 `mcp__poe_build__start_generation_run` 返回的 draft skeleton 填写安全
输出，调用 `mcp__poe_build__validate_generation_draft` 并传相同 offense group/name。服务端观察最终 PoB 的
...
随后调用 `mcp__poe_build__evaluate_generation_candidate`，传本 run 凭据、candidate、版本、本次反馈模式及
同一 offense group/name。正式 Judge 捕获不可变快照并在独立引擎评估；`mcp__poe_build__evaluate_build`、
`mcp__poe_build__pinnacle_readiness` 和 Agent 自拟评分不能代替它。重复 enabled 组、无合法 host 的普通多主动组、
...
| 确需推倒核心机制 | 在同一 run 内重建；局部技能组问题先用精确原子编辑，只有方向确需重建时才用 `mcp__poe_build__new_build` | 最多两次核心机制级修复/重建；仍无法证明但硬合法可交付待验证候选，已确认真实失败且补救失败则停止交付 |
