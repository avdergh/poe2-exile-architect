...
`mcp__poe_build__save_final_build_artifact(run_id, run_token, candidate_id, attempt_index)`。每个 run 只保存
...
`mcp__poe_build__verify_lifecycle_stage(stage, artifact_id=..., detail="compact", strict_mode=<本次模式>)`。
...
- **普通 Create：**将安全最终对象提交 `mcp__poe_build__validate_generation_output`，按返回字段路径修正；
  它不消费 run。通过后将同一最终对象提交 `mcp__poe_build__complete_generation_review`，由插件在受管用户
  数据目录原子写入并消费 review。`HumanReviewPacket` 只表示可进入人工验收，不代表人已认可
  构筑。不要自行定位或编辑运行文件。
- **Blind Create：**按 [blind-mode.md](blind-mode.md) 完成身份回读并提交
  `mcp__poe_learning__submit_learning_create_result`，进入 Compare；不无条件执行普通
  `mcp__poe_build__complete_generation_review`，不执行用户导出或本任务清理。其他自动 Create 同样不进行面向
...
| 用户选择 | 执行 | 完成条件与清理 |
| --- | --- | --- |
| 仅本地 | `mcp__poe_build__export_final_pob_artifact(artifact_id, format="both", name=...)`，再 `mcp__poe_build__export_final_build_artifact(artifact_id, name, author, description)` | XML、import code、`.build` 三个文件均成功才完成；不调用整包或 poe.ninja 发布入口。本地工具没有整包清理回执，保留现场 |
| 本地 + poe.ninja | 只调用一次 `mcp__poe_build__export_final_build_package(artifact_id, name, author, description)`，生成三个本地文件并上传同一已验证 PoB code | 四项全部成功且 `runtimeCleanupReady=true` 后，才调用 `mcp__poe_build__cleanup_completed_task_runtime(task_kind="generation", task_id=artifact_id)` |
