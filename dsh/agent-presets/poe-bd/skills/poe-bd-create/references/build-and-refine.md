...
`mcp__poe_build__apply_build_mutation_batch(batch_kind=..., ...)` 只执行Agent已决定的机械修改，不能装入搜索、
optimizer或依赖新group fingerprint才能决定的编辑。不要把整个BD混进一个批次。

| 职能 | 本阶段用途 |
| --- | --- |
| `bootstrap` | `mcp__poe_build__new_build`可为首项，设置职业、升华及最终目标等级 |
| `mechanism_shell` | 普通宝石主输出时恰含一次`set_main_skill`，只初始化一次，并设置显式武器槽；副技能留给来源盘点后的skill_loadout |
| `required_gear` | 本流程用于已决定的机制武器；非武器来源件用独立`mcp__poe_build__equip_item/mcp__poe_build__equip_jewel` |
| `skill_loadout` | 来源盘点后仍缺失职责的技能组 |
| `passive_delta` | Agent选定的局部节点分配/取消，不全局重排 |
| `ordinary_gear` | 机制件锁定后补普通装备，使用工具允许的显式槽位 |
| `config` | 每批一次`mcp__poe_build__set_config`；最终战斗假设在验证阶段锁定 |

只有以`mcp__poe_build__new_build`开始的bootstrap可省略`expected_state_hash`；其后串联上一操作的
...
箭袋是例外：不放入mutation批次；独立`mcp__poe_build__equip_item`省略slot自动识别引擎`Weapon 2`，不能传`Quiver`。

失败只回滚当前职能事务；`rolledBack=true`才表示恢复成功。`recoveryRequired=true`时停止普通
事务，通过`mcp__poe_build__new_build` bootstrap显式恢复。`mcp__poe_build__craft_item`或镶嵌工具返回的特殊来源
`craftReceiptRef`在独立/批量`mcp__poe_build__equip_item`时原样传入，不能手写或省略。

## 2. 先落实机制来源，再补技能职责

1. 先设置最终目标等级，落实Blueprint及组件决策采用的非可选`unique_enabler`、会授予技能的
   升华/核心被动。逐件读当前静态全文并验证；不能把整个adopted package的每个组件自动视为采用。
2. 调用`mcp__poe_build__list_skill_groups`，按实际source、slot、active effect和enabled状态盘点已有清图、Boss、
...
3. 装备/升华/被动提供主输出时跳过`mechanism_shell`，落实提供者后用
   `mcp__poe_build__set_skill_group_state(make_main=true, ...)`选择真实来源组。禁止创建普通宝石代理，或为编辑性、
   辅助审计和Spirit比例禁用正确来源。
...
| 内容阶段 | `mcp__poe_build__plan_gear(stage=...)` | 元素/非CI混沌规划目标 |
| --- | --- | --- |
| 剧情 | `campaign` | 30% / 0% |
...
资源等缺口；明确内容需求才覆盖到75%，不是各阶段都要求满抗。`mcp__poe_build__scaffold_gear`仅供中间态计算，
最终必须替换全部`Scaffold ...`占位物品。

`mcp__poe_build__plan_gear/mcp__poe_build__optimize_item/mcp__poe_build__craft_item/mcp__poe_build__rank_upgrades`使用`acquisition_profile="realistic_trade"`，
...
`mcp__poe_build__optimize_flask(slot, base?, ilvl, strategy)`，按职责选`recovery/sustain/instant`，证据不足默认
`recovery`。Flask是Magic最多1前缀/1后缀，不套普通Rare的3/3链。完成第一版职业、升华、等级、
技能组、必要装备、核心天赋和基础配置后进入核心检查，不能拿空骨架验收。

## 4. 核心可行性：在护符、额外珠宝和镶嵌精修前

用现有`mcp__poe_build__list_skill_groups/mcp__poe_build__get_build_stats/mcp__poe_build__get_defenses`及精确机制/等级查询，不新增评分器。
...
- 对每个清图/Boss/setup-payoff核心组，重新读取fingerprint并用`mcp__poe_build__set_skill_group_state(make_main=true)`
...
| 方向 | 操作与采用依据 |
| --- | --- |
| 精确搜索 | `mcp__poe_build__search_passives/mcp__poe_knowledge__search_mods/mcp__poe_knowledge__search_items`不设固定候选条数上限；默认返回量不是上限。选定后用`mcp__poe_build__get_passive/mcp__poe_knowledge__get_item`等精确详情，必要时扩展结果或改查询。天赋搜索与详情均由Build服务读取当前状态 |
| 武器/黄装/天赋 | 用单槽`mcp__poe_build__optimize_item/mcp__poe_build__craft_item`、`mcp__poe_build__plan_gear`和局部`mcp__poe_build__optimize_passives`；禁止`mcp__poe_build__optimize_build`与`mcp__poe_build__optimize_passives(reset=true, points=0)`全局重排 |
| 辅助 | 对每个用户可编辑组及`noSupports=false`的核心`Tree:*`/`Item:*`组，用group_index/新鲜fingerprint调用`mcp__poe_build__optimize_supports`，按职责比较完整当前与候选组合；同目标完整测量、合法性不回归且净正收益才要求修改，可以纯移除辅助，不机械要求五辅 |
| Spirit机会成本 | 定向查询该等级的精魂/保留搭配，验证等级、Spirit、实际作用。使用率不高于80%时实测与Blueprint职责一致的持久技能：有正收益且不破坏机制则采用；无可容纳/有价值选项则记录留余量理由，不塞无关技能凑比例 |
| 普通暗金 | `mcp__poe_build__relevant_uniques`发现后用`mcp__poe_knowledge__search_uniques/mcp__poe_knowledge__get_unique`读全文；静态排除不兼容项后最多实测3个最高相关候选。属性/抗性被破坏时，最多围绕该件重规划一次黄装后决定，不展开分支树；Family必需件不受3件限制 |
| 珠宝 | 暗金/radius/Time-Lost都位置化评估；Time-Lost先`mcp__poe_knowledge__search_mods`确认精确mod ID，再`mcp__poe_build__optimize_jewel(selected_mod_ids=...)`构造。已分配槽替换用`mcp__poe_build__evaluate_jewel_socket`，普通rare珠宝在Family暗金珠宝之后评估 |
| 护符 | 容量读最终PoB `CharmLimit`并封顶3；腰带缺`Charm Slots`为unknown，不按0。90级默认三槽腰带/三个护符质量目标。`mcp__poe_build__optimize_charm`生成Magic 1/1，Normal Charm只作candidate |
| 镶嵌 | `mcp__poe_build__plan_item_sockets_batch(slot_socket_counts, goals)`评估全部可镶嵌装备，局部重测用`mcp__poe_build__optimize_item_sockets`；两者共用回执登记。`socketed/partial_socketed`方案须实际`mcp__poe_build__equip_item`并原样带craftReceiptRef；状态解释见验证reference |

精魂技能在资料中未出现不等于不需要；按主技能/职业补查。资源剩余不是线性可兑换的收益，Spirit
机会成本不能变成低利用率硬失败。辅助允许分别作用于同组宿主和输出，但每个辅助至少有一个PoB
确认的实际作用目标；数值能力仍跟随选中的精确输出。辅助搜索同时从当前与空组合出发，结果不证明
...
新增路径可用`mcp__poe_build__alloc_passive(path_attribute=...)`明确选择力量/敏捷/智慧，已有属性点用
`mcp__poe_build__set_passive_attribute(node, attribute)`改选，归入`passive_delta`小事务；从`mcp__poe_build__get_passive`读取选项，
不按职业默认猜选，也不为补属性重排整树。

用`mcp__poe_build__list_jewel_sockets`盘点：已分配槽必须填真实珠宝，经`mcp__poe_build__evaluate_jewel_socket`比较后用
`mcp__poe_build__equip_jewel`显式填入/替换，正收益允许采用；该操作不花天赋点，不属于新增槽位决策。
写后必须通过物品来源、暗金数量、活动Spec读回与回滚核验。目标≥90才强制额外槽审计；核心与重要支撑
天赋完成后以精确node ID声明`protected_node_ids`，为已选下一颗珠宝调用`mcp__poe_build__evaluate_next_jewel_socket`，
比较全部当前可达槽及安全单点叶节点的等点边际，不重排树。正收益只能经
`mcp__poe_build__apply_next_jewel_socket_decision`原子应用，再在新state重审；不能先手工点额外槽后填珠宝来绕过审计。
无正收益或无可达槽才结束。
...
`mcp__poe_build__alloc_passive`无weapon-set参数，单状态DPS不能成为双状态强信号。记录State_A/State_B未分别验证
的caveat，不为单状态面板放弃用户明确的切换方向。终局目标有诊断价值时可用`mcp__poe_build__pinnacle_readiness`，
剧情阶段不受它的终局门槛驱动。

## 6. 锁定来源辅助并转入最终验证

`noSupports=false`的真实`Tree:*`/`Item:*`组可配置辅助，容量取最终角色等级对应的技能基础等级。
目标等级须早于授予技能的升华/被动分配；配置来源辅助后不要再`mcp__poe_build__set_level`。物品来源辅助必须在
机制装备、Rune和词条最终锁定后，用新鲜fingerprint和实际source group调用
`mcp__poe_build__configure_source_skill_supports(source_group_index, supports, expected_fingerprint, expected_state_hash?)`。
