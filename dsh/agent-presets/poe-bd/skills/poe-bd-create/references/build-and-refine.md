...
| 方向 | 操作与采用依据 |
| --- | --- |
| 精确搜索 | `mcp__poe_build__search_passives/mcp__poe_knowledge__search_mods/mcp__poe_knowledge__search_items`不设固定候选条数上限；默认返回量不是上限。选定后用`mcp__poe_knowledge__get_passive/mcp__poe_knowledge__get_item`等精确详情，必要时扩展结果或改查询 |
...
用`mcp__poe_build__list_jewel_sockets`盘点：已分配槽必须填真实珠宝。目标≥90才强制额外槽审计；核心与重要支撑
...
`mcp__poe_build__apply_next_jewel_socket_decision`原子应用，再在新state重审；无正收益或无可达槽才结束。
