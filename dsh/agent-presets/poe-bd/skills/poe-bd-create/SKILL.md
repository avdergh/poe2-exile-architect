...
默认模式必须渐进查询
`mcp__poe_knowledge__query_research_memory(response_profile="create_compact")`。每次首查都会建立新的 bounded retrieval
...
`mcp__poe_knowledge__construct_research_execution_contract`，按 `caseProfiles` 选择与用户目标最一致的设计案例，并对
...
`mcp__poe_learning__query_learning_memory`、提交 `mcp__poe_learning__submit_learning_create_result`）。Blind 或其他自动 Create 不执行面向
...
4. 按下面的 MCP 工具清单查询资料，基于查询结果设计候选构筑方向，不要临时猜工具名。普通模式
   ...
   - 先用 `mcp__poe_knowledge__graph_tool_query(tool_name="search_graph_components", ...)` 发现候选，再用
     `mcp__poe_knowledge__graph_tool_query(tool_name="resolve_graph_component", ...)` 确认用户指定的职业/升华 stable key；
   ...
   - Family discovery 首次查询使用 `detail_level="family"`，传当前 `class_key`、`ascendancy_key`、
     ...
     `mcp__poe_knowledge__query_research_memory` ToolReference；ToolReference summary 必须说明该案例是佐证、补充还是与
     授权 lane 冲突，以及它如何改变设计或验证计划。普通 Create 在数据库存在 2 个案例时必须读
     2/2；存在 3 个及以上时至少读 3 个总案例。Blind Create 保持 claim 绑定的单案例隔离，不做此
     对照。普通 Create 的每次 Research 首查传本次 `mcp__poe_build__start_generation_run` 的 `run_id` 作为
     `run_ref`；continuation 只传服务端 cursor。`run_ref` 只关联同一生成任务，不改变 scope/case
     权威。
     读完初始 lane 与对照 lane 后调用
     `mcp__poe_knowledge__construct_research_execution_contract(authoritative_dedupe_query_refs,
     ...
     对最终采用的 discovery 调用
     `mcp__poe_build__record_generation_family_discovery(run_id, run_token, family_discovery_ref,
   ...
   - 选定 Family 后，先读取 `familyRecordCoverage.requiredDeepReadRecordIds` 中的全部记录；该集合
     ...
     都会使 `mcp__poe_build__validate_generation_draft` 返回 `research_required_records_not_read`。之后若具体设计缺口
     尚未关闭，再使用
   ...
   - `mcp__poe_knowledge__get_meta_builds` / `mcp__poe_knowledge__get_meta_archetype_trends` 返回 unavailable 时不阻塞 Create，也不把空结果
     解释为“该流派不存在”。先复核当前 PoB、corpus、Graph 与 Research；仍有机制或样本缺口时，
     再查官方补丁/官方数据/PoE2 Wiki，最后查当前赛季 poe.ninja、pobb.in 等样本，并用
     `mcp__poe_build__import_build` / 当前 PoB 复核。联网证据只写入 `toolReferences`、`rationaleSummary`、
   ...
   - 调用 `mcp__poe_build__validate_generation_blueprint(run_id, run_token, blueprint_draft)`。只有返回 accepted 后才可
     开始 `mcp__poe_build__new_build`、技能/天赋变更、暗金评估、`mcp__poe_build__plan_gear`、craft 或装备写入。把返回的
...
5. 使用 `mcp__poe_build__apply_build_mutation_batch(batch_kind=..., ...)` 把 Agent 已经决定的机械操作按职能拆成
   小事务：`bootstrap` 只设置 `mcp__poe_build__new_build`（可选首项）、职业和等级；普通宝石承担主输出时，
   `mechanism_shell` 只设置该唯一主技能和显式武器槽，所有副技能都必须等来源核对后再通过
   `skill_loadout` 添加。主输出由装备、升华或被动授予时跳过 `mechanism_shell`，先用
   `required_gear`/独立 `mcp__poe_build__equip_item`/`passive_delta` 落实武器和来源提供者，再读取真实来源组并用
   `mcp__poe_build__set_skill_group_state(make_main=true, ...)` 设为主技能；不得创建普通宝石代理。随后按需使用
   ...
   **箭袋（Quiver）不在任何批次白名单**，引擎把它映射为 `Weapon 2` 槽（completeness 层叫
   ...
   `mcp__poe_build__set_config` 仍只修改显式传入字段，`mcp__poe_build__apply_combat_profile` 则完整覆盖自己的 Boss tier 与六个布尔
   ...
   **任何 `skill_loadout` 之前必须完成一次技能来源核对**：
   ...
   - 立即调用 `mcp__poe_build__list_skill_groups`，以实际 `source`、`slot`、主动技能和 enabled 状态确认装备、升华、
   ...
   - 只有来源核对后仍缺失的职责才进入 `skill_loadout`。之后任何已知会授予或移除技能的装备、
    ...
    的技能基础等级决定。
    单阶段 Create 必须先 `mcp__poe_build__set_level(<最终目标等级>)`，再分配授予技能的升华/被动节点，最后读取
    `mcp__poe_build__list_skill_groups` 的新鲜 fingerprint 并调用 `mcp__poe_build__configure_source_skill_supports`；配置后禁止再次
    调用 `mcp__poe_build__set_level`。物品来源技能必须在机制装备、Rune 和装备词条最终确定后再配置；主输出来自
   ...
   新鲜 fingerprint 的 `mcp__poe_build__optimize_supports`；
   ...
   `mcp__poe_build__configure_source_skill_supports(source_group_index, supports, expected_fingerprint,
   ...
   不支持的来源必须保留真实技能并记录 modelability caveat，不能复制普通技能绕过。
    质量收尾必须包含一次普通候选暗金评估轮：调用 `mcp__poe_build__relevant_uniques`（按活动主技能 scaling 匹配
    ...
    `mcp__poe_build__equip_jewel` 实测 delta，数值以引擎为准。只有装备方案和实测结果锁定后才调用 `mcp__poe_knowledge__get_prices`，
    价格只写入 ToolReference、注意事项和最终用户价格表，不参与暗金、黄装、符文、药剂或 Family
    的选择。unique/radius jewel（含 Time-Lost）必须位置化评估。Time-Lost 词缀先用
    `mcp__poe_knowledge__search_mods` 取得精确 mod ID，再由 `mcp__poe_build__optimize_jewel(selected_mod_ids=[...])` 合法构造；不得手写
    未验证词缀。已分配槽中的替换用 `mcp__poe_build__evaluate_jewel_socket`，尚未开启的槽统一走下述受保护全槽
    审计；普通 rare jewel 只在 Family 暗金珠宝完成评估后用 `mcp__poe_build__optimize_jewel` 生成。
   ...
   未给预算时 `mcp__poe_build__plan_gear`、`mcp__poe_build__optimize_item`、`mcp__poe_build__craft_item` 与 `mcp__poe_build__rank_upgrades` 都使用默认
   ...
   **进入护符、额外珠宝和 Rune/Soul Core 精修前，必须完成一次核心可行性检查。**此时应已装备
   ...
   现有 `mcp__poe_build__list_skill_groups`、`mcp__poe_build__get_build_stats`、`mcp__poe_build__get_defenses`、等级/机制资料和必要的精确机制查询，
   ...
   - 检查当前属性、装备需求、抗性和未封顶 Spirit 账本。当前已装备物品或已启用宝石的属性需求
     ...
     确定性矛盾同样必须先修；
   - 对每个承担清图、Boss 或 setup/payoff 的核心技能组，先重新读取 `mcp__poe_build__list_skill_groups`，用新鲜
     fingerprint 通过 `mcp__poe_build__set_skill_group_state(make_main=true, ...)` 临时选为计算组，再读取该组的
...
6. 补齐真实装备系统，而不是只填十个基础装备槽：
   - 实际装备当前阶段生命药剂和魔力药剂。Family 明确指定暗金药剂时直接装备并验证；普通药剂
     使用 `mcp__poe_build__optimize_flask(slot, base?, ilvl, strategy)`，根据 Research 职责选择
   ...
   - 护符容量以最终 PoB `CharmLimit` 为准并封顶 3；腰带缺少 `Charm Slots` 是 unknown，不能按 0。
     90 级 Create 默认把三槽腰带和三个护符作为质量目标；用 `mcp__poe_build__optimize_charm` 生成合法 Magic
     1 前缀/1 后缀方案。Normal Charm 只能进入 `candidate`，不能进入推荐成品；
   - 调用 `mcp__poe_build__list_jewel_sockets`；分配了珠宝孔就必须用 `mcp__poe_build__optimize_jewel` / `mcp__poe_build__equip_jewel` 填入真实珠宝。
     仅目标等级 ≥90 强制执行额外珠宝槽审计；低于 90 级不作为质量门禁，只在机制需要时评估。
     核心与重要支撑天赋完成后，先把不得为珠宝牺牲的精确节点 ID 列为
     `protected_node_ids`，再为下一颗真实候选调用 `mcp__poe_build__evaluate_next_jewel_socket`。该工具比较这颗珠宝的
     全部当前可达槽，只允许当前安全单点叶节点参与等点替换；它不替 Agent 选择珠宝，也不重排天赋
     树。正收益只能用 `mcp__poe_build__apply_next_jewel_socket_decision` 原子应用，随后必须在输出 state 上重新审计，
     直到当前候选无正收益或没有可达槽。`policy_limited/inconclusive` 可继续 Judge，但交付保持
     candidate；不得用成熟案例槽数、固定轮数或最低槽数代替当前 state 回执；
   - 用一次 `mcp__poe_build__plan_item_sockets_batch(slot_socket_counts, goals)` 评估全部可镶嵌装备；`socketed` 结果
     的 item/craftReceiptRef 必须实际 `mcp__poe_build__equip_item`；`partial_no_positive` 表示保留原孔容量、已装入正收益
     Rune 且其余孔无正收益，`no_positive/not_applicable` 才能全部留空。检查以孔容量对比带可信
     craft receipt 的实际 Rune 数；只有 `Rune:` 声明而没有匹配效果/receipt 不算已镶嵌。不要为
     镶嵌重做整件装备，也不要机械套用带 Perfect Essence 和腐化的终局 `mcp__poe_build__craft_item` 结果。
7. 正式最终检查前先固化战斗假设：对终局/Boss 目标调用一次
   `mcp__poe_build__apply_combat_profile(tier=<与用户目标一致>，...开关...,
   expected_state_hash=<最新stateHash>)`（或用 `mcp__poe_build__set_config` 传相同 key）
   ...
   `mcp__poe_build__inspect_generation_checkpoint(strict_mode=<本次反馈模式>,
...
8. 使用 checkpoint 的 `buildSummary` 复读职业、升华、等级和主技能；只有需要完整局部诊断时
   ...
   Research 深读完成且最终候选摘要形成后，按 `mcp__poe_build__start_generation_run` 返回的完整 camelCase draft
   skeleton 填写 prompt、candidate、versionContext 和 premise decisions，并在首次正式 Judge 前按
   当前机制修订调用一次 `mcp__poe_build__validate_generation_draft(run_id, run_token, agent_output_draft,
...
9. 修复 checkpoint `preflight.blockingIssues` 后才进入正式 Judge；完全重复的 enabled skill group、
    ...
    最多在其前调用一次活动快照
    `mcp__poe_build__verify_lifecycle_stage(..., detail="compact", strict_mode=<本次反馈模式>,
...
10. 调用 `mcp__poe_build__evaluate_generation_candidate(run_id, run_token, candidate_id, version_context,
...
17. 普通交互 Create 的 artifact 保存成功后，按开始时已经确定的交付方式执行，不再询问。Blind/
    ...
    - **仅本地**：调用 `mcp__poe_build__export_final_pob_artifact(artifact_id, format="both", name=...)` 生成 PoB XML
      和 PoB import code，再调用 `mcp__poe_build__export_final_build_artifact(artifact_id, name, author, description)`
      生成官方 `.build`。不得调用 `mcp__poe_build__export_final_build_package`，也不得调用任何 poe.ninja 发布入口；
      三个本地文件均成功才算本地交付完成；任一失败时展示对应 errorCode 并保留现场；
    - **本地 + poe.ninja**：只调用一次
      `mcp__poe_build__export_final_build_package(artifact_id, name, author, description)`，由现有工具生成三个本地文件并
...
`mcp__poe_knowledge__construct_research_execution_contract`（比较 case profiles、生成 package 合同；不自动组装 BD）｜
`mcp__poe_knowledge__query_public_learning_memory`（普通 Create 用，lesson 必须重新验证）｜
...
`mcp__poe_build__plan_gear(acquisition_profile="realistic_trade", locked_slots=[...])`/`mcp__poe_build__optimize_item`/
`mcp__poe_build__optimize_flask`/`mcp__poe_build__optimize_charm`/`mcp__poe_build__plan_item_sockets_batch`/`mcp__poe_build__evaluate_next_jewel_socket`/
`mcp__poe_build__apply_next_jewel_socket_decision`/
`mcp__poe_build__configure_source_skill_supports`/`mcp__poe_build__scaffold_gear`/局部
`mcp__poe_build__optimize_passives`｜
...
`mcp__poe_build__start_generation_run`/`mcp__poe_build__record_generation_family_discovery`/`mcp__poe_build__validate_generation_blueprint`/
`mcp__poe_build__validate_generation_draft`/`mcp__poe_build__validate_generation_output`/
`mcp__poe_build__complete_generation_review`（Phase 5 run，
不依赖仓库工作目录）｜`mcp__poe_build__save_final_build_artifact`（先于 complete_review）｜
`mcp__poe_build__list_final_build_artifacts`/`mcp__poe_build__load_final_build_artifact`｜`mcp__poe_build__export_final_pob_artifact` +
`mcp__poe_build__export_final_build_artifact`（仅本地）｜`mcp__poe_build__export_final_build_package`（本地 + poe.ninja）；单格式补导/
诊断用 `mcp__poe_build__export_final_pob_artifact`/`mcp__poe_build__export_final_build_artifact`/
...
- `toolReferences`：每项为对象 `{toolName, queryRef, summary}`；其中
  `mcp__poe_knowledge__query_research_memory` 条目的 `queryRef` 必须覆盖 `researchMemoryUse.dedupeQueryRefs` 与
  `comparisonDedupeQueryRefs` 的全部引用，且必须包含一条 `mcp__poe_knowledge__get_freshness_report` 条目（服务端硬性要求）
- `memoryReferences`（可省略；helper 从 typed `researchMemoryUse` 生成完整去重并集）
- `researchMemoryUse`
- `researchExecutionPlan`：必须引用本轮 `mcp__poe_knowledge__construct_research_execution_contract` 的 `contractRef`，
  选择的 design case 必须等于最终授权 lane；包含全部 package 决定与跨案例完整搭配计划
- `mechanismBlueprintRef` / `mechanismBlueprint`：装备制作前经
  `mcp__poe_build__validate_generation_blueprint` 接受的自由机制蓝图及其薄证据/覆盖索引；ToolReference 必须包含
  同一 `blueprintRef`
- `rationaleSummary`
- `versionContext`
- `noRawMaterial`
...
`mcp__poe_build__start_generation_run` / `mcp__poe_build__record_generation_family_discovery` / `mcp__poe_build__validate_generation_blueprint` /
`mcp__poe_build__validate_generation_draft` / `mcp__poe_build__validate_generation_output` /
`mcp__poe_build__complete_generation_review` 按工作流
