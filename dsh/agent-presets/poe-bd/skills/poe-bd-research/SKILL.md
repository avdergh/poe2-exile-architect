...
- 工具未直接列在当前上下文时，先使用宿主提供的标准 tool discovery / tool search，按精确名称
  查找 `mcp__poe_knowledge__query_research_memory` 和 `mcp__poe_knowledge__graph_tool_query`；不要仅凭首屏工具列表断言 MCP 不可用。
...
**显式意图优先于预检菜单**：只要用户明确给出了案例数量、分析目的或任何运行意图（例如"抓 5 个案例来分析"、"研究一下这个流派"），就直接按意图执行完整流程（`--limit N` 或对应参数），**不得推荐或执行 `--dry-run` 预检**。预检不产生任何知识（不建队列、不研究、不 accept），只用于用户无参数且明确想先验证链路时；把它当成用户实际分析的替代品是错误。若用户明确要分析但语气含糊，按合理默认（`--limit 20` 小批量）执行，并说明已开始真实入队，而不是先问要不要预检。
...
1. `repoRoot` 优先使用当前宿主 MCP 注册中 `poe-knowledge-mcp`（或任一 `poe-*-mcp`）行的
   `cwd`；若宿主不暴露启动
...
7. 只有最终 status 显示全部请求案例均为 `accepted` 后，调用
   `mcp__poe_build__cleanup_completed_task_runtime(task_kind="research", task_id=<queue 返回的 runId>, allow_rejected=<true 仅当 run 含永远无法 accept 的 rejected 案例且知识已入库>)`。它删除 queue、
...
- 补充研究（对同一 source 案例的追加 run，例如补录珠宝、暗金或 Spirit 维度）无需手动复刻身份：
  ...
  无法确认既有 Family 的 primary 集合时，先用 `mcp__poe_knowledge__query_research_memory` 的 familyRecordCoverage
  核对，再写身份记录。
...
- 机制签名词缀驱动记录因果结论时，必须独立成记录（gear_synergy/mechanic_chain）或至少进入
  ...
  **至少一种 corroboration**（source_artifact / pinned_pob_static / typed_graph /
  ...
  无 corroboration 的条目），并尽量带 `mcp__poe_knowledge__lookup_mechanic` 的可追溯 sourceRef；wiki 不可用时以
...
- 宽召回 memory 查询使用 `mcp__poe_knowledge__query_research_memory(detail_level="summary",
