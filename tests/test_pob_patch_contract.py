...
"""
    ...
    def test_source_support_patches_are_tracked_and_match_the_vendored_core(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_name:
            ...
            for relative in tracked_files:
                self.assertEqual(
                    (POB_ROOT / relative).read_text(encoding="utf-8").replace("\r\n", "\n"),
                    (replay_root / relative).read_text(encoding="utf-8").replace("\r\n", "\n"),
                    relative,
                )
    ...
    def test_item_source_supports_are_isolated_but_source_less_groups_still_share(self) -> None:
        ...
        self.assertIn(
            ...
            "otherSocketGroup.slot == group.slot then",
            calc_setup,
        )
        self.assertIn(
    ...
    def test_real_engine_isolates_item_supports_spirit_roundtrip_and_cross_links(self) -> None:
        ...
        with self._item_source_test_engine() as engine:
            ...
            self.assertEqual(["Precision I"], engine.call("test_applied_supports", index=1)["supports"])
            self.assertEqual([], engine.call("test_applied_supports", index=2)["supports"])
            self.assertEqual([], engine.call("test_applied_supports", index=3)["supports"])
            ...
            self.assertEqual(["Herald of Ash", "Precision I"], [gem["name"] for gem in groups[0]["gems"]])
            self.assertEqual(["Herald of Ice"], [gem["name"] for gem in groups[1]["gems"]])
            self.assertEqual(["Herald of Thunder"], [gem["name"] for gem in groups[2]["gems"]])
        ...
        with self._item_source_test_engine() as engine:
            ...
            groups = list_skill_groups(engine)["groups"]
            blood = next(group["index"] for group in groups if group.get("activeSkill") == "Herald of Blood")
            configured = engine.call(
                "configure_source_skill_supports",
                index=blood,
                supportGemIds=[precision_id],
            ...
            for index in (1, 2, 3):
                self.assertEqual(
                    ["Deadly Herald"],
                    engine.call("test_applied_supports", index=index)["supports"],
                )
            self.assertEqual(
