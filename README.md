...
当前最成熟的产品能力是成熟 BD 研究提取：从 poe.ninja 或本地 PoB code 建立队列，让 agent 一案一轮分析，把可复用但不可复制的机制知识写入长期记忆。
...
仍在建设中的能力包括完整 BD 自动生成、官方 `.build` 导出、Critic repair loop 和 reward memory。README 不把这些描述成已完成产品。
...
安装器会链接 `poe-bd-creator-plugin/skills/poe-bd-research`，不会覆盖已有真实目录；卸载只删除自己创建的 symlink/junction。
