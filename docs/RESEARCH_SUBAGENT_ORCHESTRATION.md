...
- Controller 只负责 typed queue/resume、worker 编排、等待、status、可选回访和最终汇总；不 claim、
  不读取案例证据、不编辑 review、不 accept。派发显式点名 Worker Skill，并只携带 opaque runRef；
  fork 必须包含用户本次研究请求/授权，不能使用无父上下文派发。
...
- 回访默认关闭。用户明确要求时，完成案例先进入待回访队列；Controller 在当前 run 所有
  ...
  并要求只反馈不修改。反馈确认或放弃前保留 runRef；知识补录由 Controller 建新的 supplement
...
- inspect/read/search、Memory/Graph 查询和 typed validation 可并发。
...
- 正式 accept 与 retry 共享进程级全局 RLock；跨进程文件锁按 Research Memory DB 派生。锁内
