...
0.5.5数据/模型升级已完成：基础语料固定RePoE bdfed992/4.5.5.2，Headless PoB固定dev ce566eac，
登记模型`0.23.1-dev.20260910`与游戏数据0.5.5，runtime contract 10。八个本地补丁可重建；新增
17核心、原生配置块、默认攻击组、物品授予技能稀疏等级与合法降级已实测。三个升级阶段均独立
交叉审查，最终全量3640项与compute123项全部通过，静态及发布门禁通过。
本地插件`0.5.0+codex.20260913135152`已安装启用；实际cache的3565发布文件与已审ZIP一致，
新PoB、语料和图选择均从实际安装路径验证，441个用户知识/BD/导出/run文件指纹不变。
已有0.5.4研究来源不改标，现有未建模机制不提升权限；后续新任务可开始0.5.5 Research，尚未启动
50样本批次。完整证据见[升级验收](../../tmp/upgrade-055-20260913/REPORT.md)。

用户已确认当前95级Gemling Twister完成生成与验证，因此不再把跑完该BD列为下一步。此为用户
验收确认，本次未另行读取最终artifact，不补造其数值或正式回执，也不由单例完成宣称质量提升。
9月9–11日实跑后的共享修复已交付至`5c9d160`，本地插件`0.5.0+codex.20260911141041`已安装验收。

原始聊天确认：9月8日Spark真实基线完成，9月10日Amazon Ice Shot正式Judge及交付验证通过，
后续Gemling亦获用户确认。首轮真实Create与交付验收已经完成，不重新安排。受控增益与稳态成本
评估没有查到完成证据，属于另外的专项，不能把它们与普通实跑混为一谈。
首轮审查计划第6阶段的专项盘点已完成：现有导出provider、Windows/Codex和局部多进程覆盖已核销；
S6-01的Checkpoint跨引擎缓存问题已修复并安装到`0.5.0+codex.20260913053937`。v11按引擎及
实际PoB进程实例隔离，检查过程中上下文失效则拒绝旧结果；保留同进程同状态复用，动态刷新不再
持有全局缓存锁。新增13项合成回归和3项真实PoB恢复/双会话测试；全量范围3498项最终逐项通过。
初轮82项因测试目录位于仓库、1项因插件清单版本不同而失败，修正环境/同步版本后83项定向复验
全部通过；没有放宽断言。Ruff/Mypy、发布合同、实际安装缓存恢复检查和3554文件一致性通过，
141项用户存储/导出指纹保持。未改PoB/Lua/数值算法，未重复compute golden；旧任务不强停，新任务加载新版。
真实PoB双会话、运行时重建和强制终止本测试子进程后的显式快照恢复已补验。随后用户将本轮
范围收窄为一次轻量Create流程错误恢复验证，不开展Research入库中断或长期混合压力专项。
保存阶段故障注入通过：写manifest失败后不留下半成品，原run/candidate/attempt可重试保存，
可信评估列表不增加；凭run、候选和轮次查到唯一成品后，现有load/export可继续交付。
这项验证复用既有Judge/引擎测试替身，只验证错误后的保存、发现、载入与导出控制流，不声明
新增真实BD数值或完整Review已验收；未新增恢复工具、后台循环、数据库或生产代码。
常用恢复方式：保留runId/runToken、candidateId、已返回的attemptIndex及精确输出；先查同run成品，
已保存则继续该artifact的验证/Review/导出。未保存且原进程可信快照仍在时，修正错误后重试保存。
进程快照丢失、run过期或最后Judge是否消费不明时，不凭猜测复用成功结论，继续按现有证据门禁处理。
详见[专项核对报告](../../tmp/stage6-audit-20260913/REVIEW.md)。不把4个Research补研gap解释成
阶段4/5首轮未完成；下文各日期段保留当时状态，历史“尚未完成”或旧Phase7“6/10”不作为当前进度。

## 普通 Create 实跑与共享工具修复（2026-09-09）

普通 Create 实跑确认并修复了四项共享工具缺口：`get_passive` 移入持有当前 PoB 的 Build 服务；
新增显式路径属性选择与已有属性点局部改选；已有珠宝槽写入复用合法性、来源和活动 Spec 读回事务，
树珠宝进入共享 Checkpoint/Judge 与 artifact 检查；暗金效果比较对称分离来源投影与剪贴板中的
种类标记，修复 Heart of the Well 的合法词条误拒。已有槽填入与新增槽审计分开，额外槽的受保护
等点收益与原子应用门禁保持。标记修复保留变体选择、完整词条多重集、roll、指纹与特殊来源审计。

此前三项修复已在重载后的普通 Create 中实际使用：属性改选与已有槽 Prism 写入成功；随后发现并
修复 Heart 标记问题。新增定向43项通过，包含 Huntress/Amazon、Sorceress/Stormweaver 的真实
PoB正反例。3314项full仅出现一次更新期间的入口版本不同步，修正后相关8项全过，功能代码未再改。
静态检查、发布合同、四域MCP和Research读回通过；本轮完整compute的126项全部通过。
...
C1使用`support_audit_v5`比较完整当前组合与完整候选，保留当前辅助协同和所有活动技能设置。
当前不可用的辅助按官方移除事件与精确ID排除，既不签数值无收益，也不强制采用。
新发现的失效项可由Agent提交绑定版本、官方与独立内容审读的会话内排除，重算完整有效组合，
采用后重审并继续交付；资料/输入纠错不占质量探索、核心重建或正式Judge额度。
辅助授予的效果先追溯独立合法宿主，不能自授予或循环授权；候选搜索排除耗尽可用生命的保留方案，
继续比较其他合法辅助。CI 同样适用生命保留硬门槛，释放 Spirit 不能抵消可用生命为零。
...
普通Create实测另补上使用条件合同：从PoB已应用支持读取`HasUsageCondition`，逐effect/support
effect ID锁定条件。最小搜索集合保留原条件辅助，新增/移除/替换条件不再被面板DPS自动授权。
Agent明确改组后可重新审计；旧无条件证据的数值回执必须重验，条件实际覆盖率仍需独立验证。
...
6. Agent 先调用 `inspect_generation_checkpoint` v6。该工具按语义 build-state hash 合并
   ...
   完整且缺口由原生PoB明确识别为数值模型缺失的 `capability_gap` 可作为 unknown 进入 Judge，并保持 candidate。
   代理生成与Herald不能用负载速度或零DPS替代缺失频率；声明持续/耐久/持续伤害物体而缺少其
   持续时间及持续伤害模型时，正的普通武器命中也不构成完整伤害排序依据；可保留结构与unknown
   候选，但不能改称数值验证通过。技能价值不按建模覆盖降权，有依据的Agent情景粗估独立展示。
